...
let
  capture = "tmux capture-pane -p -t nix-test";
  visible =
    command:
    mkAction "machineAssertion" {
      node = "machine";
      code = "machines[\"machine\"].wait_until_succeeds(${builtins.toJSON command})";
    };
in
{
  machine = {
    getByText =
      ...
      mkLocator {
        type = "machineText";
        inherit text;
        toBeVisible = visible ("${capture} | grep -F -- ${lib.escapeShellArg text}");
      };
    ...
    */
    getByPattern =
      ...
      mkLocator {
        type = "machinePattern";
        inherit pattern;
        toBeVisible = visible ("${capture} | grep -E -- ${lib.escapeShellArg pattern}");
      };
